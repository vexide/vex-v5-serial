...
async fn main() {
    ...
    device.send_packet(GetSystemVersionPacket::new(())).await.unwrap();
    let response = device.recieve_packet::<GetSystemVersionReplyPacket>(Duration::from_millis(100)).await.unwrap();
